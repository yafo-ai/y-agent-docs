...
Y-Agent 支持与多种供应商模型的集成，支持对接当前主流的大型模型。这包括但不限于本地Ollama、vLLM、Xinference部署的私有模型如 Llama 3 和 Qwen 2等，国内提供的公共模型服务例如 DeepSeek、SILICONFLOW、Kimi、智谱 AI 和百度千帆、讯飞星火、通义千问，以及国际知名的公共模型服务如 OpenAI、Azure OpenAI、Anthropic 和 Gemini 等。

Y-Agent 目前仅支持集成文本模型和视觉模型；接入方式我们适配了OpenAI、Ollama、ZhipuAI等SDK，建议使用OpenAI的方式接入。

关于Embedding向量模型，Y-Agent 已支持的 Embedding 模型接口规范可以在[系统配置]-[Embedding]配置项选择vllm、xinference接入方式，建议使用vllm方式。

>对于排序算法、语音识别、语音合成、图像生成、视频生成等模型的集成，目前尚无集成计划。

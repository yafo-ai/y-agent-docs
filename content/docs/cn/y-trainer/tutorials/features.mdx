...
```bash
# 启用NLIRG算法的训练命令示例
python -m training_code.start_training --use_NLIRG 'true' [其他参数...]
```
...
```bash
# LoRA训练示例
python -m training_code.start_training --use_lora 'true' [其他参数...]
```
...
# 或使用默认路径（输出目录下）
python -m training_code.start_training --use_tensorboard 'true' --output_dir /path/to/output
```

无论是否指定 TensorBoard 日志路径，系统都会在 `--output_dir` 指定的路径下自动生成训练损失图表。

例子如图:
![tensorboard example](imgs/tensorboard-example.png)
